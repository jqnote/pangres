...
class UpsertQuery:
    ...
    def _create_mysql_query(self, values: list, if_row_exists: str, incr_columns: list = []) -> MySQLInsert:
    ...
    def create_query(self, db_type: str, values: list, if_row_exists: str, incr_columns: list):
        r"""
        ...
        try:
            return query_creation_methods[db_type](values=values, if_row_exists=if_row_exists, incr_columns=incr_columns)
        except KeyError:
            raise NotImplementedError(f'No query creation method for {db_type}. '
                                      f'Expected one of {list(query_creation_methods.keys())}')

    def execute(self, db_type: str, values: list, if_row_exists: str, incr_columns: list):
        query = self.create_query(db_type=db_type, values=values, if_row_exists=if_row_exists, incr_columns=incr_columns)
        return self.connection.execute(query)

    async def aexecute(self, db_type: str, values: list, if_row_exists: str, incr_columns: list):
        """
